...
window.addEventListener('DOMContentLoaded', function() {
    ...
    // Fonction pour afficher le modal avec le contenu spécifié
    function openModal(title, content, showRetraitOptions = false) {
        const modal = document.getElementById('myModal');
        const modalTitle = document.getElementById('modalTitle');
        const modalContent = document.getElementById('modalContent');
        const retraitOptions = document.getElementById('retraitOptions');

        modalTitle.textContent = title;
        modalContent.textContent = content;
        retraitOptions.style.display = showRetraitOptions ? 'block' : 'none';
    ...
    document.getElementById('soldeButton').addEventListener('click', () => {
        openModal('Votre Solde', `Votre solde actuel est de ${currentUser.solde} FCFA.`);
    });

    // Fonction pour afficher la section de retrait
    document.getElementById('retraitButton').addEventListener('click', () => {
        openModal('Effectuer un Retrait', 'Choisissez un montant à retirer:', true);
    });

    // Fonction pour effectuer un retrait
    function retirer(montant) {
        const retraitMessage = document.getElementById('modalContent');
